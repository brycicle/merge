<?php 
	...
	if(isset($_POST['submit'])){
		...
		if ($amount > $selected_withdraw) {
			redirect("Withdrawal.php", "Not enough si payment.");
			exit; // Exit the script
		}
		$total_result = $selected_withdraw - $amount;

		// Update the existing record with the new admin_fee value
		$sql_update = "UPDATE `genealogy` SET `si_payment`='$total_result' WHERE `usertype`='admin'";
		$sql_run_update = mysqli_query($conn, $sql_update);

		if ($sql_run_update) {
			// Insert a new record with the updated admin_fee value
			$sql_insert = "INSERT INTO `si_withdraw` (`withdrawal_type`, `withdrawer`, `amount`) VALUES ('Si Payment','$username','$amount')";
