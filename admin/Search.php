<?php 
    ...
    $sql = "SELECT id, fname, lname, username FROM register WHERE usertype = ? AND status = ? AND account = ?";
    $usertype = 'user';
    ...
    mysqli_stmt_bind_param($stmt, "sss", $usertype, $status, $account);
    ...
    if (empty($matches)) {
        ...
        foreach ($matches as $match) {
            ...
            echo "<td data-label = 'Action'><a style='color: white;' class='a' href='Banned_code_block.php?id=" . $match["id"] . "'>Block</a></td>";
            echo "</tr>";
        }
        echo "</table>";
        echo "</div>";
