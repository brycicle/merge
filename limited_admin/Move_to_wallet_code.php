...
if(isset($_POST['submit'])) {
    ...
    // Check if both DRB and pairing are greater than zero
        ...
        $sql = "SELECT `limitedadminid` FROM `genealogy` WHERE `accountid`='$accountid'";
        $result = mysqli_query($conn, $sql);

        if($result && mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $limitedadminid = $row['limitedadminid'];
            ...
            if($result_1) {
                ...
                $sql = "UPDATE `genealogy` SET `pairing`='$pairing_result', `DRB`='$DRB_result' WHERE `limitedadminid`='$limitedadminid'";
